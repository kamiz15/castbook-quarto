...
### Using Maier and Grathwohl (2006) model in PACS

### Single computing interface

The PACS user-interface for Both the empirical models are identical. This is true for both single computing interface and multiple simulation interface. The identical interface simplifies the use of models in the PACS. Therefore, the extensive details on using the  [Liedl et al. (2005)](refli2005)  model in PACS provided [here](refli) can be used also for the Maier and Grathwohl (2006).

The difference in each model interface is the model specific parameters. This can be observed in the PACS screenshot below: 
...
As explained in detail in [Liedl et al. (2005)](refli2005)  model , the resulting graph provides several interactive options to evaluate results. The screenshot below provides the output of the single computing mode of PACS.
