# v. BIOSCREEN-AT Model {.unnumbered}

## Model description

The BIOSCREEN-AT, provided in [Karanovic et al. (2007)](#refbs) Model provides an exact analytical Solution to the BIOSCREEN Model. The model provides a three-dimensional solution for transport of dissolved contaminants, incorporating natural attenuation processes. As shown in @fig-3-v-01, the source is given as a patch specified-concentration boundary condition.

![The BIOSCREEN-AT model concept](images/bioscreen/fig3-v-01.png){#fig-3-v-01 fig-align="center" width="70%"}
...
Following the approach provided in [Karanovic et al. (2007)](#refbs) the limits of integration of eq. @eq-eqbio2007 is scaled. This is required for speeding up the computation. The codes used in CAST for this purpose can be seen by clicking the _dropdown_ button
...
Eq. @eq-eqbio2007 provides the solution for $C(x,y,z)$, i.e., concentration isosurface. However, we intend to find $L_{max}$, which is a centreline maximum longitudinal distance of the specificed threshold concentration. Thus, to obtain only $L_{max}$ we fix the domain (based on [Karanovic et al. (2007)](#refbs) ) as follows:
...
The CAST interface of BIOSCREEN-AT is identical to that for other analytical/empirical models. The details of the interface and available user options can be found at [Liedl et al. (2005)](liedl2005.qmd). Below a very brief outline of the CAST interface specific to BIOSCREEN-AT is provided.


***Single computing simulation***

First all input data needs to be defined (screenshot @fig-3-v-02). Then a graph is genereated, that shows the result of the computation in comparison to the field data from the CAST database (screenshot @fig-3-v-03).

![The CAST interface for BIOSCREEN-AT data input](images/bioscreen/fig3-v-02.png){#fig-3-v-02 fig-align="center" width="70%"}

![The CAST interface for BIOSCREEN-AT output](images/bioscreen/fig3-v-03.png){#fig-3-v-03 fig-align="center" width="50%"}

Once the first computation of $L_{max}$ pressing `Generate graph` button is made, the CAST interface provides interface with **sliders** bars (@fig-3-v-04) for the most sensible model parameters.

![Interactive sliders in CAST for BIOSCREEN-AT model](images/bioscreen/fig3-v-04.png){#fig-3-v-04 fig-align="center" width="50%"}
...
The multiple computing mode can be used to visualize different own case results in one graph, while still being able to compare to the CAST database. The interface for BIOSCREEN-AT is identical to other models. User can check  [Liedl et al. (2005)](../an_model/liedl2005.qmd) on details of using the **multiple computing interface**.
...
![Multiple computing interface in CAST for BIOSCREEN-AT model](images/bioscreen/fig3-v-05.png){#fig-3-v-05 fig-align="center" width="50%"}
