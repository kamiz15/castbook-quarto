...
![Standard site dataset of CAST](images/db_f1.png){#fig-db1 fig-align="center" width="60%"}
...
**Dispersivity** is among the most critical quantity required for obtaining plume extensions. However, dispersivity data are almost never available (see @fig-db1), and diligent estimate of it needs to be made for using in models. More critical are transverse dispersivity data, mostly because of their very low magnitude. Due to this reason, **CAST** includes a dataset of dispersivity (see @fig-db3) found in the literature. 


![Transverse dispersivity dataset in CAST](images/db_f3.png){#fig-db3 fig-align="center" width="60%"}

**Dispersivity** dataset in CAST is an isolated dataset. It is only provided for reference purpose such that user can make an appropriate estimate when using models. To facilitate further, the exploratory data analysis of  dispersivity dataset is provided (see screenshot @fig-db4 ).

![Analysis of transverse dispersivity dataset](images/db_f4.png){#fig-db4 fig-align="center" width="60%"}

##### User data {#user_data}

**Additional user data** let users add their own data and compare them with the _standard dataset_. User can add data directly to the table by clicking `Add data` button (see screenshot @fig-db2 ), clicking of which opens a data input windows. This option can be tedious if many site data are to be included as the _user data._ 

User can also add their data using the `Download sample file` (see screenshot @fig-db2) button. This will download a sample data input (**.csv**) file to the user's computer. The sample file can be filled with data, and then uploaded to the CAST using the `upload` button. Users are not required to be **online** while working of the sample file. 
...
![User data input interface in CAST](images/db_f2.png){#fig-db2 fig-align="center" width="60%"}
...
**CAST** provides only a limited data analysis and visualization options. However, the provided options are certainly good enough to have an impression of site data including user data. Users are provided options for downloading data in _editable_ format (e.g., csv) through **CAST** interface (see @fig-db1 ). This simplifies users data analysis outside of **CAST**. 
...
![Bargraph of the plume length](images/db_f6.png){#fig-db6 fig-align="center" width="60%"}

##### Boxplots

Boxplot is the other plot-type that can be used to visualize site datasets in CAST. In this plot, users can observe the _mean_, _median_, _maximum_ and _minimum_ values for FOUR different site parameters: ``PLume Length`` (see screenshot @fig-db7), ``Aquifer Thickness``, ``Electron Donor``, ``Electron Acceptor`` in the plot. Note that ``Electron Donor`` refers to _Contaminant Concentration_ and ``Electron Acceptor`` the partner reactant such as Oxygen, Nitrates etc. User can interact using the  features (zoom, box select etc., @fig-db6) 

![Boxplots of several site quantities.](images/db_f7.png){#fig-db7 fig-align="center" width="50%"}
...
![Histogram and underlying distribution type of site parameters.](images/db_f8.png){#fig-db8 fig-align="center" width="70%"}
...
![Scatter plots and fit options in CAST.](images/db_f9.png){#fig-db9 fig-align="center" width="70%"}

Scatter plots are fundamental in developing models. Therefore, in CAST in addition to the visualization, different fit-types: ``Exponential``, ``Linear``, ``Power 2``, ``Power 3``) are provided between different site parameters (e.g., ``Aquifer Thickness``, ``Electron Donor``) with the ``Plume Length`` (see @fig-db9). Plume Length in CAST is the main output quantity. The main output of the Models used in CAST (see [](../an_model/an_model.qmd)) is maximum plume length or $L_{max}$.
...
![Stastiscal overview of the site data](images/db_f10.png){#fig-db10 fig-align="center" width="50%"}
