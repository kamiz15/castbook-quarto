# i. MODFLOW and MT3DMS Numerical Model {.unnumbered}
## Model Description
The numerical model in the PACS is based on the conceptual work presented in [Yadav et al. (2014)](des_num). The 2D numerical domain is vertically orineted and follows the assumptions similar to that provided in [Liedl et al., (2005,)](des_num), e.g., homogeneous, isotropic, steady-state and bi-component (Electron Donor or contaminant concentation $C_{ED}$ and Electron Acceptor or the partner reactant concentration $C_{EA}$) instantaneous reaction (see [Liedl et al., 2005](des_num) Analytical Model of the PACS- **to be linked**).  

![Numerical model conceptual set-up](images/numerical/fig5-i-01.png){#fig-5-i-01 fig-align="center" width="60%"}
...
Complete details of **MODFLOW** used for flow simulations from [here](https://www.usgs.gov/software/modflow-2005-usgs-three-dimensional-finite-difference-ground-water-model), and that of the **MT3DMS** required for transport simulations can be obtained from [here](https://hydro.geo.ua.edu/). Furthermore, FloPy (see [here](https://www.usgs.gov/software/flopy-python-package-creating-running-and-post-processing-modflow-based-models)) is used for implementing numerical model in PACS interface.


## Implementation in PACS

The code snippets below show calling of MODFLOW and MT3DMS executables in PACS. For **OFFLINE** PACS users have to link these executables as they exist in their system. Note the _double backslash (\\)_ used in assigning the directory. This has to be followed as it is required by the Python Programming standard.

```python
    exe_name_mf = copy('C:\\Users\\VB\\Desktop\\Personal-code\\PACS\\mf2005.exe', path)
    exe_name_mt = copy('C:\\Users\\VB\\Desktop\\Personal-code\\PACS\\mt3dms.exe', path)
```

The above code is in PACS directory:
``numerical_models.py``

The standard method of simulating Transport problems using MODFLOW/MT3DMS is also used in PACS. In the (standard) approach, **flow** simulations using MODFLOW is simulated first. The **transport** simulation is followed after the successful simulation of flow simulation. 

The code snippet below shows the implementation of **flow** simulation in PACS. Complete source-code is accessible from [here](https://github.com/kamiz15/Groundwater-Contamination-Website/blob/main/numerical_models.py).
...
The numerical approach for solving transport related groundwater problems comes with the advantages of being adaptable to many cases regarding their individual case features as well as high accuracy. However, this also comes with an increased computing effort, mainly dependent of the number of cells used for the model. Very fine meshes result in high computing times, which may not be suitable for the online version of the tool. It is recommended to start simulating with rather coarse meshes and then refining them after. Also, an **offline** version can be used (see [here](../../contents/offline/offline_installation.qmd) if very fine meshes for very high accuracy are wanted. The offline version also does not restrict parameter ranges in any way.  
...
@fig-5-i-02 provides the screenshot of system parameters input interface of the PACS.

![Numerical model system parameters](images/numerical/fig5-i-02.png){#fig-5-i-02 fig-align="center" width="40%"}
...
@fig-5-i-03 provides the screenshot of hydraulic and chemical parameters input interface of the PACS.

![Numerical model hydraulic and chemical parameters](images/numerical/fig5-i-03.png){#fig-5-i-03 fig-align="center" width="40%"}
...
@fig-5-i-04 provides the screenshot of the model output from PACS.

The obtained output can be saved as a *.png* file.


![Numerical model output](images/numerical/fig5-i-04.png){#fig-5-i-04 fig-align="center" width="70%"}
